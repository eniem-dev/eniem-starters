...
This boilerplate uses `eniem-feeback-collect-saas` (slug) and `Eni Feeback` (display name) as placeholders. Replace them with your own project name:

```bash
# Replace display name (e.g., "My Cool App")
grep -rl "Eni Feeback" . --exclude-dir={node_modules,.git} | xargs sed -i 's/Eni Feeback/Your App Name/g'

# Replace slug (e.g., "mycoolapp")
grep -rl "eniem-feeback-collect-saas" . --exclude-dir={node_modules,.git} | xargs sed -i 's/eniem-feeback-collect-saas/yourslug/g'
```
